...
const GradientHeading = React.forwardRef<HTMLHeadingElement, HeadingProps>(
  ({ asChild, variant, weight, size, className, children, from, to, ...props }, ref) => {
    const Comp = asChild ? Slot : "h3" // default to 'h3' if not a child
    const customGradient = from && to ? `bg-gradient-to-r ${from} ${to}` : ""
    return (
      <Comp ref={ref} {...props} className={className}>
        <span className={cn(
          headingVariants({ variant: from && to ? undefined : variant, size, weight }),
          customGradient
        )}>
          {children}
        </span>
